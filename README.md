...
# Все результаты (пагинация)
node index.js --stickers 2711 --all

# Определённое количество страниц
...
| `--pages <n>` | Количество страниц для пагинации |
| `--all` | Скачать все страницы результатов |
...
| `--delay <ms>` | Задержка между запросами в мс (по умолчанию 1500) |
| `--output <path>`, `-o` | Путь к выходному файлу (по умолчанию results.json) |
...
src/
  ...
  floatdb-client.js         — HTTP-клиент FloatDB API (search, searchAll, searchCount)
